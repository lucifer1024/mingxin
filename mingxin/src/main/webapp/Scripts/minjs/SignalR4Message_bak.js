...
function SetCurrentUserSNNOInSignalR(h, c, g, k, d, a, b, e, f, j, i, l) {
	...
	var isChatMessageAutoClear = (a == 1 ? true : false);
...
function ConnectHub() {
	...
	if (connectedCount > 2) {
		if (isMobile) {
			window.location = basePaht+"/Room/Room4Mobile.aspx?RoomId=" + roomIdInSignalR
		} else {
			window.location = basePaht+"/Room/RoomDetail.aspx?RoomId=" + roomIdInSignalR
		}
		return
	}
